...
Eine graue Hinterlegung zeigt an, dass der Nutzer keine direkte Kontrolle über die entsprechende Funktion hat.
...
\begin{tikzpicture}[%
  ...
  \node {Pre-Match-Phase /F0050/}
    child { node {Rollenwechsel /F0110/}}
    child { node {Quickstart-Vote /F0070/}
    ...
    child [missing] {}
    child { node {Zurückkehren zur Hauptseite /F0120/}
      child { node [hiddendupe] {Hauptseite}}
    }
  ;
\end{tikzpicture}
...
\begin{tikzpicture}[%
  ...
  two children at (0.5,-0.7) and (0.5,-1.4)},
  edge from parent path={(\tikzparentnode.south) |- (\tikzchildnode.west)}]
  \node {Match-Phase /F0090/}
    child { node {Zum Zuschauer wechseln /F0110/}}
    child { node {Zurückkehren zur Hauptseite /F0120/}
      child { node [hiddendupe] {Hauptseite}}
    }
    child [missing] {}
    child { node {\vires\ spielen /F0100/}
